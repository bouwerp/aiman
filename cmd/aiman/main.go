...
func run() error {
	...
	// while previewing a PTY session.
	//
	// It is also a correctness matter for the holder: opening the shared SQLite
	// file while serve or the dashboard holds it returns "database is locked",
	// and a holder that cannot start is a session that cannot start.
	...
	if len(os.Args) > 1 {
		...
		case "serve":
			return runServe()
		// `session` and `pty` are dispatched earlier, before the config and
		// database setup they do not need.
		case "context":

...
build:
	...
	go build -v -ldflags "$(LDFLAGS)" -o aiman-trigger ./cmd/aiman-trigger
	go build -v -ldflags "$(LDFLAGS)" -o aiman-gateway ./cmd/aiman-gateway
...
clean:
	@echo "Cleaning..."
	rm -f aiman aiman-trigger aiman-gateway
	rm -f coverage.out

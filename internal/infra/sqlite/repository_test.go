...
func TestSaveClearsAWSProfileWhenAWSConfigPresent(t *testing.T) {
	repo, err := NewRepository(filepath.Join(t.TempDir(), "aiman.db"))
	if err != nil {
		t.Fatalf("NewRepository: %v", err)
	}
	...
	if err := repo.Save(ctx, &domain.Session{
		ID:             "sess-aws",
		Status:         domain.SessionStatusActive,
		AWSProfileName: "aiman-legacy",
		AWSConfig:      &domain.AWSConfig{SourceProfile: "default", RoleName: "Role", Region: "eu-west-1"},
		CreatedAt:      created,
	}); err != nil {
		t.Fatalf("initial Save: %v", err)
	}
	...
	got, err := repo.Get(ctx, "sess-aws")
	if err != nil {
		t.Fatalf("Get: %v", err)
	}
	if got.AWSProfileName != "" {
		t.Fatalf("expected aws profile to be cleared, got %q", got.AWSProfileName)
	}
}
